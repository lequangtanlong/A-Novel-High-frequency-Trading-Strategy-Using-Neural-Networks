...
class Stat:
    ...
    def summary(self, capital=None, resample='5min', trading_days=365):
        ...
        rs_equity_wo_fee = pd.Series(raw_equity_wo_fee, index=dt_index)
        rs_equity = equity
        ...
        print('The number of trades per day: %.2f' % dtn)
        print('Avg. daily trading volume: %.2f' % dtq)
        print('Avg. daily trading amount: %.2f' % dta)
        ...
        if capital is not None:
            ((mid / mid[0] - 1) * 100).plot(ax=axs[0], style='grey', alpha=0.5)
            (rs_equity / capital * 100).plot(ax=axs[0])
            (rs_equity_wo_fee / capital * 100).plot(ax=axs[0])
        else:
            mid.plot(ax=axs[0], style='grey', alpha=0.5)
        ...
        position = pd.Series(self.position, index=dt_index)
        position.plot(ax=axs[1])
        axs[1].set_ylabel('Position (Qty)')
        axs[1].grid()

    def save_summary_to_file(self, capital=None, resample='5min', trading_days=365, name=''):
        dt_index = self.datetime()
        raw_equity = self.hbt.local.state.asset_type.equity(
            np.asarray(self.mid),
            np.asarray(self.balance),
        ...
        filename = datetime.datetime.now().strftime(f"summary_of_{name}_%Y%m%d_%H%M%S.txt")
        with open(filename, 'w') as f:
            f.write('=========== Summary ===========\n')
            f.write('Sharpe ratio: %.5f\n' % sr)
            f.write('Sortino ratio: %.5f\n' % sortino)
            f.write('Risk return ratio: %.5f\n' % rrr)
            if capital is not None:
                f.write('Annualised return: %.5f%%\n' % (ar / capital * 100))
                f.write('Max. draw down: %.5f%%\n' % (mdd / capital * 100))
            else:
                f.write('Annualised return: %.5f\n' % ar)
                f.write('Max. draw down: %.5f\n' % mdd)
            f.write('The number of trades per day: %.5f\n' % dtn)
            f.write('Avg. daily trading volume: %.5f\n' % dtq)
            f.write('Avg. daily trading amount: %.5f\n' % dta)

            position = np.asarray(self.position) * np.asarray(self.mid)
            if capital is not None:
                f.write('Max leverage: %.5f\n' % (np.max(np.abs(position)) / capital))
                f.write('Median leverage: %.5f\n' % (np.median(np.abs(position)) / capital))
        ...
        plt_filename = datetime.datetime.now().strftime(f"summaryEquity_of_{name}_%Y%m%d_%H%M%S.png")
